    ...
    if ($_SERVER['REQUEST_METHOD'] != 'POST') {
        ...
        if (isset($categoryName, $categoryDesc) && $categoryName != '' && $categoryDesc != '') {
		   ...
		   VALUES('" . htmlentities(strip_tags(mysqli_real_escape_string($con, $categoryName) )). "',
                ...
                echo "<div>";
                echo "<h3 class=\"error-msg\">Името на категорията трябва да бъде с дължина между [4..100]</h3>";
                echo "<h3 class=\"error-msg\">Описанието на категорията трябва да бъде с дължина между [6..255]</h3>";
                echo "</div>";
            }

        }else {
            echo "<div><h3 class=\"error-msg\">Името и описанието на категорията трябва да са попълнени</h3></div>";
        }
    }
}
